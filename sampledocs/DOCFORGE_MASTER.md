# DocForge — Master Build Document
> Document Intelligence Platform: Merge · Compress · OCR · Convert
...
### What is DocForge?
DocForge is a browser-based document toolkit that lets users perform four core operations on their documents — **Merge**, **Compress**, **OCR**, and **Convert** — without installing any software. All processing happens server-side via a FastAPI backend. No accounts required in v1.
...
### Core Value Props
- Fast, clean UI — no ads, no paywalls, no tracking
- Handles PDF, DOCX, TXT, MD in every operation
- Privacy-first — files are deleted from server after processing
...
#### MERGE
- Accept 2–20 files per merge job
...
#### COMPRESS
- Accept PDF and DOCX
- Show original size → estimated output size (before processing)
...
| PDF → DOCX | pdf2docx | Good fidelity |
| DOCX → PDF | LibreOffice headless | Most reliable conversion |
| DOCX → TXT/MD | python-docx + markdownify | |
| TXT/MD → PDF | WeasyPrint | HTML → PDF pipeline |
| MD → DOCX | pandoc (via subprocess) | Best MD → DOCX quality |
...
Landing Page
    │
    ├── User selects a tool tab: [Merge] [Compress] [OCR] [Convert]
    │
    ▼
Tool Page
    │
    ├── Drop Zone: drag & drop OR click to browse
    │       └── Files appear as cards with: name, size, type icon, remove button
    │               └── (Merge only): drag handles for reordering
    │
    ├── Options Panel (tool-specific settings)
    │       ├── Merge: output format (PDF/DOCX), separator style
    │       ├── Compress: quality level slider (Low/Med/High)
    │       ├── OCR: output format, language selection
    │       └── Convert: target format selector
    │
    ├── [Process] button
    │       └── Upload files → show upload progress bar
    │               └── Processing spinner with status message
    │                       └── Success: show output file size, [Download] button
    │                               └── Option to process another batch
    │
    └── Error States
            ├── File too large → inline error on card
            ├── Unsupported format → inline error on card
...
> v1 is stateless — no database. All state is in-memory (job dict) + filesystem (/tmp).
...
### Phase 1 — Foundation + Convert (Week 1)
...
**Step 1.3 — Convert tool (backend)**
- [ ] PDF → DOCX (pdf2docx)
- [ ] PDF → TXT (pypdf)
- [ ] PDF → MD (pypdf + formatting)
- [ ] DOCX → PDF (LibreOffice headless)
- [ ] DOCX → TXT / MD (python-docx)
- [ ] TXT → PDF (WeasyPrint)
- [ ] MD → PDF (WeasyPrint with md rendering)
- [ ] MD → DOCX (pandoc)
- [ ] TXT ↔ MD (direct text processing)
- [ ] PNG/JPG → PDF (Pillow)

**Step 1.4 — Frontend shell**
...
**Verification:** Upload a PDF → get DOCX back. Upload MD → get PDF back.

---

### Phase 2 — Merge Tool (Week 2)
...
- [ ] Size comparison display: "4.2 MB → ~1.1 MB (est.)"
...
### Phase 4 — OCR Tool (Week 3)
> Goal: extract text from scanned PDFs and images

**Step 4.1 — Backend**
- [ ] Image OCR: pytesseract → TXT/MD
- [ ] PDF OCR: pdf2image (pdftoppm) → per-page images → pytesseract → reassemble
...
- **Clean and functional** — this is a tool, not a portfolio. Whitespace over decoration.
- **No gradients, no shadows** (except subtle card borders)
- **Monochrome-first** with a single accent color (Teal — `#1D9E75`)
...
- Right: remove button (×), drag handle (merge only)

**Process Button:**
- Full-width, `44px` height, accent background, white text, `8px` radius
- Loading state: spinner + "Processing…" text, disabled
...
- [ ] Feature works end-to-end (upload → process → download)
...
### Phase 1 — Foundation + Convert
| # | Task | Status | Notes |
|---|------|--------|-------|
| 1.1a | Monorepo scaffold | ⬜ Not started | |
| 1.1b | Frontend init (React+Vite+Tailwind+shadcn) | ⬜ Not started | |
| 1.1c | Backend init (FastAPI+venv) | ⬜ Not started | |
| 1.2a | FastAPI core + CORS + health check | ⬜ Not started | |
| 1.2b | File upload handler + validation | ⬜ Not started | |
| 1.2c | Job store + /tmp manager + cleanup cron | ⬜ Not started | |
| 1.2d | Download endpoint | ⬜ Not started | |
| 1.3a | PDF → DOCX/TXT/MD | ⬜ Not started | |
| 1.3b | DOCX → PDF/TXT/MD | ⬜ Not started | |
| 1.3c | TXT/MD → PDF | ⬜ Not started | |
| 1.3d | MD → DOCX | ⬜ Not started | |
| 1.3e | IMG → PDF | ⬜ Not started | |
| 1.4a | Frontend layout + nav | ⬜ Not started | |
| 1.4b | Landing page | ⬜ Not started | |
| 1.4c | DropZone component | ⬜ Not started | |
| 1.4d | FileCard component | ⬜ Not started | |
| 1.4e | Convert tool page | ⬜ Not started | |

### Phase 2 — Merge
| # | Task | Status | Notes |
|---|------|--------|-------|
| 2.1a | PDF merge (backend) | ⬜ Not started | |
| 2.1b | DOCX/TXT/MD merge (backend) | ⬜ Not started | |
| 2.2a | Sortable file list (frontend) | ⬜ Not started | |
| 2.2b | Merge page (frontend) | ⬜ Not started | |

### Phase 3 — Compress
| # | Task | Status | Notes |
|---|------|--------|-------|
| 3.1a | PDF compress via ghostscript | ⬜ Not started | |
| 3.1b | DOCX image compress | ⬜ Not started | |
| 3.2a | Compress page (frontend) | ⬜ Not started | |

### Phase 4 — OCR
| # | Task | Status | Notes |
|---|------|--------|-------|
| 4.1a | Image OCR (pytesseract) | ⬜ Not started | |
| 4.1b | PDF OCR (pdf2image + tesseract) | ⬜ Not started | |
| 4.1c | Searchable PDF output | ⬜ Not started | |
| 4.2a | OCR page (frontend) | ⬜ Not started | |

### Phase 5 — Polish + Deploy
| # | Task | Status | Notes |
|---|------|--------|-------|
| 5.1 | Error handling audit | ⬜ Not started | |
| 5.2 | Mobile responsive audit | ⬜ Not started | |
| 5.3 | Rate limiting | ⬜ Not started | |
| 5.4 | Railway deploy (backend) | ⬜ Not started | |
| 5.5 | Vercel deploy (frontend) | ⬜ Not started | |
...
*DocForge Master Build Document — keep this file updated as the single source of truth.*
